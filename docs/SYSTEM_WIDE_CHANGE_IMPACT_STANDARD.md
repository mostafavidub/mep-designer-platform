...
Version 4.41.2 requires automated Staging completion to submit valid numeric engineering-basis values rather than nonnumeric acknowledgements. Version 4.41.1 synchronized build identity and runtime Rule Book revisions. Version 4.41.0 added locked rule `MEP-UNIFIED-MODEL-001`: individual rooms, network edges, calculations, plans, risers and schedules must be projections of one graph-native engineering model. Connected endpoints with zero plan branches, orphan calculations or divergent Plan/Riser/Schedule identities fail closed.
