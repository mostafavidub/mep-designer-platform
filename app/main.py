...
class User(Base):
    __tablename__='users'; id:Mapped[int]=mapped_column(Integer,primary_key=True); email:Mapped[str]=mapped_column(String(255),unique=True,index=True); password_hash:Mapped[str]=mapped_column(String(255),default='anonymous'); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
class Project(Base):
    __tablename__='projects'; id:Mapped[int]=mapped_column(Integer,primary_key=True); user_id:Mapped[int]=mapped_column(ForeignKey('users.id')); name:Mapped[str]=mapped_column(String(255)); status:Mapped[str]=mapped_column(String(50),default='awaiting_upload'); questions:Mapped[list]=mapped_column(JSON,default=list); current_question:Mapped[int]=mapped_column(Integer,default=0); answers:Mapped[dict]=mapped_column(JSON,default=dict); analysis:Mapped[dict]=mapped_column(JSON,default=dict); current_revision:Mapped[int]=mapped_column(Integer,default=0); last_error:Mapped[str]=mapped_column(Text,default=''); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
class Revision(Base):
    __tablename__='revisions'; id:Mapped[int]=mapped_column(Integer,primary_key=True); project_id:Mapped[int]=mapped_column(ForeignKey('projects.id')); revision_no:Mapped[int]=mapped_column(Integer); status:Mapped[str]=mapped_column(String(50),default='queued'); feedback:Mapped[str]=mapped_column(Text,default=''); pdf_path:Mapped[str]=mapped_column(Text,default=''); error:Mapped[str]=mapped_column(Text,default=''); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
class RuleCandidate(Base):
    __tablename__='rule_candidates'; id:Mapped[int]=mapped_column(Integer,primary_key=True); project_id:Mapped[int]=mapped_column(Integer); feedback:Mapped[str]=mapped_column(Text); candidate_rule:Mapped[str]=mapped_column(Text); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
Base.metadata.create_all(engine)

COMMON=[('location','پروژه در کدام کشور و شهر قرار دارد؟'),('occupancy','کاربری دقیق ساختمان چیست؟'),('codes','مبنای طراحی کدام مقررات و استانداردهاست؟'),('floors','تعداد طبقات، زیرزمین/پارکینگ و بام را دقیق بفرمایید.'),('units','تعداد کل واحدها و تعداد واحد در هر طبقه چقدر است؟'),('typical','کدام طبقات تیپ هستند و کدام پلان متفاوت دارند؟'),('heights','ارتفاع طبقات و وضعیت سقف کاذب را بفرمایید.'),('shafts','شفت‌ها و رایزرهای موجود قطعی هستند یا اجازه پیشنهاد داریم؟'),('roof','روی بام چه فضاها یا تجهیزاتی دارید؟'),('language','زبان خروجی را مشخص کنید؛ پیشنهاد: توضیحات فارسی و Tagهای فنی لاتین.')]
...
app=FastAPI(title='EngiTools'); app.add_middleware(SessionMiddleware,secret_key=SESSION_SECRET,same_site='lax'); app.mount('/static',StaticFiles(directory='app/static'),name='static'); templates=Jinja2Templates(directory='app/templates')
...
def analyze_project_job(project_id):
    ...
    try:
        ...
        p.analysis={'discipline':discipline,'file_count':len(files),'files':[analyze_dxf(x) for x in files]}; p.status='asking'; p.current_question=0; p.answers={'discipline':discipline}; p.last_error=''; db.commit()
...
def run_design(project_id,revision_id):
    ...
    try:
        ...
        payload={'project_id':str(p.id),'discipline':discipline,'architecture_dir':str(pdir/'input'),'answers':p.answers,'plan_analysis':p.analysis,'rulebook_path':RULEBOOK_PATH,'revision':r.revision_no,'revision_instructions':r.feedback}
        resp=requests.post(CAD_DESIGNER_URL+'/design',json=payload,timeout=3600); resp.raise_for_status(); data=resp.json(); src=Path(data['pdf_path']); out=pdir/'output'/f'rev_{r.revision_no:03d}'; out.mkdir(parents=True,exist_ok=True); dst=out/'design.pdf'; shutil.copy2(src,dst)
...
def project_page(pid:int,request:Request):
    ...
    revisions=db.query(Revision).filter(Revision.project_id==p.id).order_by(Revision.revision_no.desc()).all(); q=p.questions or []; current_question=q[p.current_question] if p.current_question<len(q) else None; discipline=(p.answers or {}).get('discipline',(p.analysis or {}).get('discipline','mechanical')); cfg=DISCIPLINES.get(discipline,DISCIPLINES['mechanical'])
    response=templates.TemplateResponse('project.html',{'request':request,'p':p,'revisions':revisions,'current_question':current_question,'question_count':len(q),'discipline':discipline,'cfg':cfg}); db.close(); return response
...
def get_pdf(pid:int,rev:int,request:Request):
    ...
    return FileResponse(r.pdf_path,media_type='application/pdf',filename=f'EngiTools_{pid}_R{rev}.pdf')
