...
def home(request: Request):
    ...
    db = Session()
    projects = db.query(Project).filter(Project.user_id == u.id).order_by(Project.id.desc()).limit(6).all()
    db.close()
...
def new_project(request: Request, name: str = Form(...)):
    u = current_user(request)
    db = Session()
    p = Project(user_id=u.id, name=name, questions=QUESTION_LIST, status='awaiting_upload')
    db.add(p)
    db.commit()
    db.refresh(p)
    pid = p.id
...
def project_page(pid: int, request: Request):
    ...
    revisions = db.query(Revision).filter(Revision.project_id == p.id).order_by(Revision.revision_no.desc()).all()
    q = p.questions or []
    current_question = q[p.current_question] if p.current_question < len(q) else None
    response = templates.TemplateResponse('project.html', {
        'request': request,
        'user': u,
        'p': p,
        'revisions': revisions,
...
def answer(pid: int, request: Request, answer: str = Form(...)):
    ...
    if idx < len(qs):
        ...
        p.status = 'ready_to_design' if p.current_question >= len(qs) else 'asking'
        db.commit()
    db.close()
    return RedirectResponse(f'/projects/{pid}', 303)
...
def design(pid: int, request: Request):
    ...
    r = Revision(project_id=p.id, revision_no=rev_no, status='queued')
    db.add(r)
    p.status = 'queued'
    db.commit()
    db.refresh(r)
    rid = r.id
    db.close()
    threading.Thread(target=run_design, args=(pid, rid), daemon=True).start()
    return RedirectResponse(f'/projects/{pid}', 303)
...
def feedback(pid: int, request: Request, feedback: str = Form(...)):
    ...
    if not p:
        raise HTTPException(404)
    if any(x in feedback for x in ['همه پروژه', 'همیشه', 'باید در تمام', 'رول بوک', 'Rulebook']):
        db.add(RuleCandidate(project_id=p.id, feedback=feedback, candidate_rule=feedback))
    rev_no = (p.current_revision or 0) + 1
    r = Revision(project_id=p.id, revision_no=rev_no, status='queued', feedback=feedback)
    db.add(r)
    p.status = 'queued'
    db.commit()
    db.refresh(r)
...
def get_pdf(pid: int, rev: int, request: Request):
    ...
    if not p:
        raise HTTPException(404)
    r = db.query(Revision).filter(Revision.project_id == p.id, Revision.revision_no == rev).first()
    db.close()
