(()=>{
  ...
  function render(d){
    ...
    if(d.status==='ready'){const downloadUrl=d.download_url||d.output_url||d.pdf_url;modalBar.style.width='100%';modalTitle.textContent='خروجی آماده است';modalBody.innerHTML=summary(d)+`<div class="modal-ready"><div class="modal-ready-icon">✓</div><h3>طراحی آماده شد</h3>${downloadUrl?`<a class="btn primary wide" href="${downloadUrl}" download>دانلود DXF</a>`:''}</div>`;return}
