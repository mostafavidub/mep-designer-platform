"""Coordinate / Fixture / Equipment Isolation v1.
...
def _inside(bounds, point):
    return bool(
        bounds and len(bounds) == 4 and point
        and float(bounds[0]) <= point[0] <= float(bounds[2])
...
def _choose_level(row, levels):
    ...
    if len(contained) > 1:
        ...
        if len(contained) == 1 or _diag(contained[0]) < _diag(contained[1]) * 0.98:
...
def install(main_auto_module):
    if getattr(main_auto_module, "_coordinate_isolation_v1_installed", False):
        return
    ...
    # Preserve provenance in the PMM without changing existing public fields.
    ...
    def fixture_rows_isolated(auto):
        ...
        if len(rows) == len(detections):
            for row, detection in zip(rows, detections):
                row["source_file"] = detection.get("source_file")
                row["level_authority_id"] = detection.get("level_authority_id")
                row["level_local_position"] = detection.get("level_local_position")
                row["coordinate_isolation_status"] = detection.get("coordinate_isolation_status")
        return rows

    pmm._fixture_rows = fixture_rows_isolated
    main_auto_module._coordinate_isolation_v1_installed = True
