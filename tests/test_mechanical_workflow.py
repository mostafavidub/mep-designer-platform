...
class MechanicalWorkflowTests(unittest.TestCase):
    ...
    def test_scope_detects_levels_and_vertical_system(self):
        p = self.project()
        scope = build_scope(p)
        self.assertEqual(scope['all_levels'], ['همکف', 'بام'])
    ...
    def test_preflight_requires_project_water_rain_and_gas_values(self):
        p = self.project({'gas':'تأیید'})
        missing = required_basis_questions(p)
        self.assertEqual(missing, ['water_inlet_pressure','rainfall_intensity','gas_pressure'])
        ...
        self.assertTrue(all(q['input_type']=='radio' and len(q['options'])>=4 for q in p.questions))
    ...
    def test_preflight_does_not_accept_unknown_as_numeric_fact(self):
        p = self.project({'gas':'خیر','water_inlet_pressure':'نامشخص','rainfall_intensity':'unknown'})
        self.assertEqual(required_basis_questions(p), ['water_inlet_pressure','rainfall_intensity'])

    def test_preflight_passes_only_after_explicit_numeric_values(self):
        p = self.project({
            'gas':'تأیید','water_inlet_pressure':'2.8 bar','rainfall_intensity':'95 mm/h','gas_pressure':'21 mbar'
        })
        self.assertEqual(required_basis_questions(p), [])
        self.assertFalse(ensure_required_basis_questions(p))

    def test_basis_question_options_are_suggestions_not_defaults(self):
        for key, spec in REQUIRED_BASIS_QUESTION_SPECS.items():
            self.assertGreaterEqual(len(spec['options']), 4)
            self.assertNotIn('default', spec['question'].lower())
            self.assertNotIn('خودکار', spec['question'])
    ...
    def test_approved_flag_is_respected_only_with_complete_basis(self):
        p = self.project({'gas':'خیر','water_inlet_pressure':'2.5 bar','rainfall_intensity':'90 mm/h'})
        create_proposal(p)
