...
def test_active_runtime_and_public_version_contract_are_identical():
    assert PLATFORM_RELEASE == CAD_API_VERSION == "18.2.0"
    assert MECHANICAL_PIPELINE_VERSION.endswith("v18.2")
...
def test_release_record_and_current_docs_do_not_drift():
    ...
    assert "18.2.0" in matrix and PRODUCTION_CAD_ENTRYPOINT in matrix
    assert "Rule Book version: 4.1" in standard
