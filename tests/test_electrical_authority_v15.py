...
from cad_engine.electrical_v1.release_contract import release_contract_status
from cad_engine.electrical_v1.acceptance_pipeline import run_acceptance_electrical_pipeline
from tests.test_electrical_project_driven_v1 import make_architecture, fully_evidenced_config


class ElectricalAuthorityTests(unittest.TestCase):
    def test_release_contract_modules_are_available(self):
        ...
        self.assertTrue(status["checks"]["north_from_architecture_evidence"])
    ...
    def test_missing_north_is_reported_but_never_fabricated(self):
        with tempfile.TemporaryDirectory() as td:
            source = Path(td) / "arch.dxf"; output = Path(td) / "electrical.dxf"
            make_architecture(source)
            report = run_acceptance_electrical_pipeline(source, output, fully_evidenced_config())
