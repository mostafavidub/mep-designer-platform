...
from cad_engine import main_v10_2 as fixed
v10 = fixed.v10
...
class ElectricalCadV10Tests(unittest.TestCase):
    def _source(self, path):
        ...
        for text,p in [
            ('آشپزخانه',(98,8)),('اتاق خواب',(96,10)),('هال و پذیرایی',(102,10)),
            ('حمام',(99,6)),('شفت',(100,8)),('آسانسور',(103,8)),
        ]: msp.add_text(text).set_placement(p)
        msp.add_text('پلان جانمایی پارکینگ').set_placement((125,0))
        msp.add_text('پلان معماری پشت بام').set_placement((70,0)); msp.add_text('بام').set_placement((72,10))
        doc.saveas(path)
    ...
    def test_end_to_end_electrical_generates_complete_rulebook_dxf(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(dst.exists()); self.assertEqual(meta['v10_final_qa']['score_10'],10.0)
            doc=ezdxf.readfile(dst); self.assertEqual(len(doc.audit().errors),0)
            names={x.name for x in doc.layouts}
            self.assertIn('E-SLD-RISER',names); self.assertIn('E-SCHEDULE',names)
            for prefix in ('E-L-','E-P-','E-F-','E-D-'): self.assertTrue(any(x.startswith(prefix) for x in names))
    ...
    def test_routes_are_orthogonal(self):
        ...
        msp.add_lwpolyline([(0,0),(2,0),(2,3)],dxfattribs={'layer':'ENGITOOLS-E-WIRE'}); self.assertTrue(v10._orthogonal_routes(msp))
        msp.add_lwpolyline([(0,0),(1,1)],dxfattribs={'layer':'ENGITOOLS-E-WIRE'}); self.assertFalse(v10._orthogonal_routes(msp))
