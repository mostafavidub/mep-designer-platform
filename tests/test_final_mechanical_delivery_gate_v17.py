...
class FinalMechanicalDeliveryGateV17Tests(unittest.TestCase):
    def test_design_basis_is_fail_closed_when_user_inputs_missing(self):
        ...
        self.assertIn('cooling_system',basis['missing'])
        self.assertIn('heating_system',basis['missing'])
        self.assertIn('water_inlet_pressure',basis['missing'])
    ...
    def test_design_basis_requires_project_specific_pressures_and_rainfall(self):
        ...
        basis=resolve_design_basis(project,{
            'city':'Tehran','cooling_system':'wall_mounted_split_ac','heating_system':'package_radiator',
            'gas_service':True,'water_inlet_pressure':'2.5 bar','rainfall_intensity':'75 mm/h'
        })
        self.assertEqual(basis['status'],'INPUT_REQUIRED')
        self.assertIn('gas_service_pressure',basis['missing'])
        basis=resolve_design_basis(project,{
            'city':'Tehran','cooling_system':'wall_mounted_split_ac','heating_system':'package_radiator',
            'gas_service':True,'gas_service_pressure':'22 mbar','water_inlet_pressure':'2.5 bar','rainfall_intensity':'75 mm/h'
        })
        self.assertEqual(basis['status'],'PASS')

    def test_pipeline_requirements_do_not_invent_optional_systems_from_room_type(self):
        architecture={'rooms':[{'id':'K1','type':'kitchen'},{'id':'L1','type':'living'}]}
        recognition={'detections':[]}
        req=derive_pipeline_requirements(architecture,recognition,design_basis={})
        systems=set(req['project_systems'])
        self.assertNotIn('gas',systems)
        self.assertNotIn('heating',systems)
        self.assertNotIn('cooling',systems)
    ...
    def test_pipeline_requirements_activate_only_locked_optional_systems(self):
        ...
        recognition={'detections':[]}
        req=derive_pipeline_requirements(architecture,recognition,design_basis={
            'cooling_system':'wall_mounted_split_ac','heating_system':'package_radiator','gas_service':True
        })
        systems=set(req['project_systems'])
        self.assertTrue({'gas','heating','cooling'} <= systems)
    ...
    def test_manifest_contains_only_project_justified_families(self):
        ...
        basis=resolve_design_basis(project,{
            'city':'Tehran','cooling_system':'wall_mounted_split_ac','heating_system':'package_radiator',
            'gas_service':False,'water_inlet_pressure':'2.5 bar','water_service_mode':'direct_city'
        })
        req=derive_system_requirements(project,basis)
        manifest=build_reference_driven_manifest(project,req,basis)
        ...
        self.assertNotIn('WATER_SERVICE_CALC',families)
        required={'COVER','GENERAL_NOTES','SANITARY_VENT','WATER','HEATING','SPLIT_AC','PLUMBING_RISER'}
    ...
    def test_manifest_adds_water_service_only_when_project_requires_it(self):
        ...
        basis={'basis':{'water_service_mode':'tank_pump'}}
        req={'project_systems':['cold_water','hot_water','sanitary','vent'],'by_level':{'GROUND':['cold_water','hot_water','sanitary','vent']}}
        manifest=build_reference_driven_manifest(project,req,basis)
        families=[x['family'] for x in manifest['sheets']]
        self.assertIn('WATER_SERVICE_CALC',families)
        self.assertTrue(any(x['family']=='WATER' and x['level']=='SERVICE' for x in manifest['sheets']))
    ...
    def test_pipeline_rejects_unrouted_or_unsized_topology_edges(self):
        result={
            ...
            'recognition':{'detections':[]},'requirements':{'project_systems':['sanitary']},
        ...
        self.assertIn('missing_project_endpoint_evidence:sanitary',qa['errors'])

    def test_pipeline_rejects_route_sizing_without_real_load_and_slope(self):
        result={
            'architecture':{'rooms':[{'id':'R1'}],'plans':[{'plan_id':'P1'}],'primary_floor_plan_ids':['P1']},
            'recognition':{'detections':[{'id':'F1','type':'wc','category':'fixture','plan_id':'P1'}]},
            'requirements':{'project_systems':['sanitary']},
            'topology':{'edges':[{'id':'E1','system':'sanitary'}],'quality':{'cross_plan_edges':0},'unresolved':[]},
            'routing':{'routes':[{'id':'R1','edge_id':'E1','system':'sanitary','plan_id':'P1','points':[(0,0),(1,0)],'length':1.0}], 'rejected':[], 'quality':{'cross_plan_routes':0}},
            'sizing':{'segments':[{'route_id':'R1','system':'sanitary','size_mm':75,'downstream_load':0,'slope_percent':0}]},
            'annotations':{'annotations':[{'route_id':'R1'}]},
            'hvac':{'status':'PASS','equipment':[],'routes':[],'quality':{'cross_plan_routes':0,'out_of_bounds':0}},
            'design_basis':{},
        }
        qa=validate_pipeline(result)
        self.assertEqual(qa['status'],'FAIL')
        self.assertIn('sanitary_route_without_slope',qa['errors'])
        self.assertIn('route_sizing_without_project_load',qa['errors'])

    def test_approved_manifest_rejects_generated_extra_sheet(self):
        report={'composition':{'manifest':[
            {'family':'SANITARY_VENT','level':'GROUND','purpose':'PLAN','code':'M-101'},
            {'family':'GAS','level':'GROUND','purpose':'PLAN','code':'M-141'},
        ]}}
        approved=[{'family':'SANITARY_VENT','level':'GROUND','purpose':'PLAN','code':'M-101'}]
        qa=validate_approved_manifest(report,{'_approved_drawing_manifest':approved})
        self.assertEqual(qa['status'],'FAIL')
        self.assertTrue(any(x.startswith('generated_unapproved_sheets:') for x in qa['errors']))

    def test_approved_manifest_accepts_exact_family_level_purpose_match(self):
        rows=[{'family':'WATER','level':'GROUND','purpose':'PLAN','code':'M-111'}]
        qa=validate_approved_manifest({'composition':{'manifest':rows}},{'_approved_drawing_manifest':rows})
