...
def solve_annotations(plan, requests, config):
    ...
    occupied=list(plan.get('obstacles') or []); placed=[];unplaced=[]
    for request in sorted(requests,key=lambda x:(-float(x['priority']),x['id'])):
        ...
        for dx,dy in config['candidate_offsets']:
            ...
            if _inside(box,plan['bounds']) and not any(_overlap(box,other,float(config['clearance_model_units'])) for other in occupied):
