...
The policy is a living canonical contract. Historical revisions remain in Git and
in immutable evidence artifacts; the active validator itself has no engine version.
"""
...
def validate_repository_governance(root: Path = ROOT) -> dict:
    ...
    if [x.get('id') for x in steps] != list(range(1, 11)):
        errors.append('ten_step_sequence_invalid')
    for step in steps:
        artifact = root / str(step.get('artifact') or '')
        if not artifact.is_file():
            errors.append(f"governance_artifact_missing:{step.get('id')}:{step.get('artifact')}")
    ...
    for capability in registry.get('capabilities') or []:
        ...
        if capability.get('status') != 'LOCKED':
            errors.append(f'capability_not_locked:{cid}')
        if not capability.get('requirements'):
            errors.append(f'capability_requirements_empty:{cid}')
        tests = capability.get('tests') or []
        if not tests:
            errors.append(f'capability_tests_empty:{cid}')
        for test in tests:
            if f'def {test}(' not in test_text:
                errors.append(f'locked_test_missing:{cid}:{test}')
    project = json.loads((root / 'standards/projects/project-1.contract.json').read_text())
    if project.get('contract_status') != 'LOCKED':
        errors.append('project_contract_not_locked')
    forbidden = set((project.get('input') or {}).get('forbidden_generation_inputs') or [])
    if 'reference_mechanical_dxf' not in forbidden:
        errors.append('reference_mechanical_not_forbidden')
    baseline = json.loads((root / 'standards/golden/project-1.baseline.json').read_text())
    if baseline.get('status') != 'APPROVED':
        errors.append('golden_baseline_not_approved')
    snapshot_path = root / str(steps[7].get('artifact') or '')
    snapshot = json.loads(snapshot_path.read_text())
    if snapshot.get('status') != 'APPROVED' or snapshot.get('all_required_gates') != 'PASS':
        errors.append('release_snapshot_not_approved')
    changes = json.loads((root / 'standards/contract-changelog.json').read_text())
    if not any(x.get('status') == 'APPROVED' for x in changes.get('changes') or []):
        errors.append('approved_contract_change_missing')
    return {
        'status': 'PASS' if not errors else 'FAIL',
        'errors': sorted(set(errors)),
        'steps_checked': len(steps),
        'capabilities_checked': len(registry.get('capabilities') or []),
    }
...
def validate_release_against_contract(report: dict, project_contract: dict | None = None, baseline: dict | None = None) -> dict:
    ...
    baseline = baseline or _load('standards/golden/project-1.baseline.json')
    errors = []
    if report.get('status') != 'PASS':
        errors.append('release_status_not_pass')
    for gate in project_contract.get('required_gates') or []:
        status = str((report.get(gate) or {}).get('status') or 'MISSING').upper()
        if status != 'PASS':
            errors.append(f'required_gate_not_pass:{gate}:{status}')
    composition = report.get('composition') or {}
    boards = composition.get('boards') or {}
    manifest = composition.get('manifest') or []
    split_visual = report.get('split_ac_visual_qa') or {}
    split_boards = split_visual.get('boards') or []
    ...
    for name, minimum in (baseline.get('minimums') or {}).items():
        if actual.get(name, 0) < minimum:
            errors.append(f'golden_minimum_regression:{name}:{actual.get(name, 0)}<{minimum}')
    ...
    if dict(families) != expected:
        errors.append('golden_family_manifest_changed_without_migration')
    return {
        'status': 'PASS' if not errors else 'FAIL',
        'errors': sorted(set(errors)),
        'actual': actual,
