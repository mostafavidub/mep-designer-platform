...
validated by v19 before a mechanical artifact can be issued. Legacy v17 remains
...
def _v19_payload(answers: dict, plan_analysis: dict) -> dict:
    ...
    return {
        ...
        "network_level_assignments": _first_value(contract.get("network_level_assignments"), plan_analysis.get("network_level_assignments_v19"), answers.get("_network_level_assignments")),
        "network_design_basis": _first_value(contract.get("network_design_basis"), plan_analysis.get("network_design_basis_v19"), answers.get("_network_design_basis")),
...
def design_mechanical_authority_site(src: Path, dst: Path, answers: dict | None = None, plan_analysis: dict | None = None) -> dict:
    ...
    if network_authority.get("status") != "PASS":
        ...
        return {"status": "FAIL", "stage": "v19_network_authority_gate",
    ...
    if traceability.get("status") != "PASS":
        return {"status": "FAIL", "stage": "v19_authority_input_gate",
                "network_authority_qa": network_authority,
                "v19_qa": {"status": traceability.get("status"), "traceability": traceability},
    ...
    if authority_errors:
        ...
        return {"status": "FAIL", "stage": "v19_authority_release_gate",
                "network_authority_qa": network_authority, "v19_qa": result,
    ...
    # leave a legacy-only DXF behind after the v19 gate has rejected the run.
    backup = None
    if dst.exists():
        fd, name = tempfile.mkstemp(prefix="engitools-v19-authority-", suffix=".dxf")
    ...
    if rendered.get("status") != "PASS":
        ...
        rendered["cad_materializer"] = "legacy-v17-shell"
        rendered["legacy_renderer_role"] = "CAD_SHELL_ONLY"
    ...
    if materialization.get("status") != "PASS":
        ...
        if backup:
            backup.unlink(missing_ok=True)
        return {"status": "FAIL", "stage": "v19_network_materialization_gate",
                "network_authority_qa": network_authority, "v19_qa": result,
                "v19_traceability_preflight": traceability,
                "v19_materialization_qa": materialization,
    ...
    rendered["network_authority_qa"] = network_authority
    rendered["v19_materialization_qa"] = materialization
    rendered["v19_qa"] = result
    rendered["v19_traceability_preflight"] = traceability
    rendered["executed_versions"] = active_version_manifest()
    rendered["pipeline_authority"] = "mechanical-v19"
    rendered["engineering_authority"] = "PMM_V3_V19"
    rendered["cad_materializer"] = "legacy-v17-shell+graph-native-v19-network"
    rendered["legacy_renderer_role"] = "CAD_SHELL_ONLY"
