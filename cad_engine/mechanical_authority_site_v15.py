...
def _served_floor_count(authority, answers):
    ...
    if explicit is not None and explicit>0:
        return int(math.ceil(explicit))
    count=0
    persian_ordinals={"اول":1,"دوم":2,"سوم":3,"چهارم":4,"پنجم":5,"ششم":6,"هفتم":7,"هشتم":8,"نهم":9,"دهم":10}
...
def _water_service_mode(answers):
    ...
    aliases={"pump":"break_tank_pump","tank_pump":"break_tank_pump","بوستر پمپ":"break_tank_pump",
             "direct":"direct_city","city":"direct_city","مستقیم":"direct_city"}
...
def _critical_water_path(pipeline):
    ...
    route=max(routes,key=lambda row:float(row.get("length") or 0))
    length=max(0.0,float(route.get("length") or 0))
    # Preliminary Darcy/Hazen proxy is intentionally exposed. Final design may
    # replace this with segment hydraulics, but never with a flat allowance.
...
def enrich_water_service(doc,msp,pipeline,authority,compose,answers):
    ...
    floor_count=_served_floor_count(authority,answers)
    floor_height=_numeric(_answer(answers,"floor_height","floor_height_m")) or 3.2
    static=max(float(floor_height),float(floor_height)*floor_count)
    residual=15.0
    critical=_critical_water_path(pipeline)
    friction=(critical or {}).get("friction_head_m")
    if friction is None:friction=max(4.0,.25*(static+residual))
    gross=static+residual+friction
    if service_mode=="inline_booster":
        pump_head=max(0,gross-service_head) if service_head is not None else None
    elif service_mode=="break_tank_pump":
        suction=_numeric(_answer(answers,"available_suction_head_m"))
        pump_head=max(0,gross-(suction or 0.0))
    else:
        pump_head=None
    autonomy_min=15
