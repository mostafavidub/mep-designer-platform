"""Rebuild support-sheet documentation from the actual engineering pipeline.

The legacy reference-parity layer inferred riser levels from every sheet manifest
row. That allowed support-sheet pseudo-levels such as DETAIL-1 to enter the riser
while an absent pipeline produced zero mapped branches. Production replaces only
those generated documentation-layer entities with a context derived from real
architectural plans and real routed engineering content.
"""
