...
def sanitize_to_approved_boards(path:Path,report:dict)->dict:
    ...
    if not boards:return {'version':'final-delivery-gate-canonical.2','status':'FAIL','errors':['no_approved_board_bounds']}
    try:
        doc=ezdxf.readfile(path); msp=doc.modelspace()
    except Exception as exc:
        return {'version':'final-delivery-gate-canonical.2','status':'FAIL','errors':['cannot_open_output_dxf'],'detail':str(exc)}
    ...
    try:
        ...
        return {'version':'final-delivery-gate-canonical.2','status':'FAIL','errors':['cannot_save_sanitized_output'],'detail':str(exc)}
...
def validate_final_delivery(path:Path,report:dict)->dict:
    ...
    try:
        ...
        return {'version':'final-delivery-gate-canonical.2','status':'FAIL','errors':['cannot_reopen_exact_output'],'detail':str(exc)}
    for e in msp:
        if not _contained(e,boards):
    ...
    return {'version':'final-delivery-gate-canonical.2','status':'PASS' if not errors else 'FAIL','errors':errors,
