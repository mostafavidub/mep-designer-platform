"""Production mechanical authority wrapper canonical.
...
canonical strengthens the approved-manifest contract so that every approved plan
...
from .mechanical_cad_preservation import design_mechanical_authority_site as _design_canonical, evaluate_architecture_preservation
from .reference_parity_engine import project_context_from_report, build_documentation_package
from .documentation_enhancer import apply_documentation_enhancements
from .final_delivery_gate import sanitize_to_approved_boards, validate_final_delivery
...
def _release_input_errors(report, pre_submission=None):
    ...
    for name,result in enrichment.items():
        ...
        record_statuses={str(rec.get('status') or '').upper() for rec in (result or {}).get('records') or []}
        disclosed_pending=bool(
            target_package_pre_submission
            and 'FAIL' not in ({status} | record_statuses)
            and (status == 'INPUT_REQUIRED' or 'INPUT_REQUIRED' in record_statuses)
...
def validate_approved_manifest(report,answers):
    ...
    if approved_raw is None:
        return {'version':'approved-manifest-gate-canonical','status':'SKIPPED','errors':[],'reason':'NO_WORKFLOW_MANIFEST_IN_DIRECT_ENGINE_CALL','generated_count':len(generated)}
    ...
    return {
        'version':'approved-manifest-gate-canonical','status':'PASS' if not errors else 'FAIL','errors':sorted(set(errors)),
...
def validate_plan_board_population(dst,report,answers):
    ...
    approved_raw=(answers or {}).get('_approved_drawing_manifest')
    if approved_raw is None:
        return {'version':'plan-board-population-canonical','status':'SKIPPED','errors':[],'reason':'NO_WORKFLOW_MANIFEST_IN_DIRECT_ENGINE_CALL'}
    composition=report.get('composition') or {}
    generated=_manifest_rows(composition.get('manifest') or [])
    ...
    try:
        ...
        return {'version':'plan-board-population-canonical','status':'FAIL','errors':['exact_dxf_reopen_failed:'+str(exc)]}
    results=[];errors=[]
    for row in generated:
    ...
    return {'version':'plan-board-population-canonical','status':'PASS' if not errors else 'FAIL','errors':errors,'boards':results,'exact_file_reopened':True}
...
def design_mechanical_authority_site(src:Path,dst:Path,answers:dict|None=None,plan_analysis:dict|None=None)->dict:
    ...
    if dst.exists():
        fd,name=tempfile.mkstemp(prefix='engitools-canonical-backup-',suffix='.dxf');Path(name).unlink(missing_ok=True);backup=Path(name);shutil.copy2(dst,backup)
    ...
    if population.get('status')=='FAIL':
        ...
        if backup:backup.unlink(missing_ok=True)
        return report
    preservation=evaluate_architecture_preservation(src,dst,report,answers=answers);report['architecture_preservation_qa_after_canonical']=preservation
