...
def _level_registry(pmm):
    ...
    for order, row in enumerate(pmm.get("levels") or []):
        ...
        if level_type in seen_types:
            errors.append("DUPLICATE_TYPED_LEVEL:" + level_type)
            continue
...
def _resolve_explicit_level(value, levels):
    ...
    if not text:
        return None
    matches = [row for row in levels if text in {row["id"], row["name"], row["type"]}]
...
def build_authoritative_topology_from_evidence(pmm, architecture, recognition, level_assignments=None):
    ...
    for (system, level_id), consumers in sorted(endpoints_by_system_level.items()):
        ...
        if system in PLUMBING_SYSTEMS and len(levels) > 1:
    ...
    if vertical_enabled:
        ...
        for system, level_ids in sorted(systems_by_level.items()):
            ...
            for lower, upper in zip(selected, selected[1:]):
                # A vertical segment carries only the demand connected above its
                # upper end. Including lower-floor fixtures would double count
                # loads that join below this segment.
    ...
    if any(node.get("kind") == "shaft" and node.get("source") != "ARCHITECTURAL_SHAFT_GEOMETRY" for node in nodes):
...
def build_authoritative_topology(src, pmm, level_assignments=None):
